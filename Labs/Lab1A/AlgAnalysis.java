...
public class AlgAnalysis {
	...
	public static void arraySortRuntime(int count, int maxSize){
		...
		double nSqr = ((double)result/(double)(i*i));
		double nLog = (double)result / ((double)i * (Math.log(i)/Math.log(2)));
		System.out.println("N: "+i+", T(n): "+result+" nanosecs, T(n)/(n*n): "+nSqr+", T(n)/(nlogn): "+nLog);
		}
	}
	...
	public static long unique1Runtime(int n){
		int[] randomArray = randomizeArray(genArray(n));
		long startTime = System.nanoTime();
